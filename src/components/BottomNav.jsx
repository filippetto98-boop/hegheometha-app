...
import { Home, Calendar, CalendarDays, Bell, Package } from 'lucide-react'
import { getStoredUser } from '../api/auth'

export default function BottomNav() {
  const user = getStoredUser()
  const isTitolare = user?.ruolo === 'titolare'
  const hasMagazzino = user?.accesso_magazzino
  ...
  const items = isTitolare
    ...
    ? [
        ...
        { to: '/calendario', icon: CalendarDays, label: 'Turni' },
        { to: '/magazzino', icon: Package, label: 'Magazzino' },
        { to: '/notifiche', icon: Bell, label: 'Notifiche' },
      ]
    : [
  ...
  const iconSize = items.length >= 5 ? 16 : 18
      ...
      <div className="flex justify-around items-center h-16 max-w-lg mx-auto">
        {items.map(({ to, icon: Icon, label }) => (
          <NavLink key={to} to={to}
            className={({ isActive }) =>
              `flex flex-col items-center gap-0.5 px-1.5 py-2 text-[10px] font-semibold transition-colors ${
                isActive ? 'text-[var(--accent)]' : 'text-[#9E96AB]'
              }`
            }>
            {({ isActive }) => (
              <>
                <Icon size={iconSize} strokeWidth={isActive ? 2.5 : 1.5} />
                <span>{label}</span>
              </>
            )}
          </NavLink>
